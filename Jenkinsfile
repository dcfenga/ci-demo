node('maven') {
    ...
    stage('Build & P3C Analysis') {
        withSonarQubeEnv('SonarQube7.6-9003') {
            sh 'mvn clean compile sonar:sonar -Pp3c-quality-profile-jdk8'
        }
    }

    stage('P3C Quality Gate') {
        waitForQualityGate abortPipeline: true
    }

    stage('Build & Default Analysis') {
        withSonarQubeEnv('SonarQube7.6') {
            sh 'mvn clean compile sonar:sonar -Pdefault-quality-profile-jdk8'
        }
    }

    stage('Default Quality Gate') {
        waitForQualityGate abortPipeline: true
    }
}